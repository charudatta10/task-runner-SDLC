...
class TestManager(unittest.TestCase):
    ...
    @patch('builtins.input', side_effect=['Test Title', 'Test Description', 'tag1,tag2'])
    @patch('logging.info')
    def test_add_task(self, mock_logging_info, mock_input, mock_save_json_file, mock_load_json_file):
    ...
    def test_main_task(self, mock_run_command):
        ctx = MagicMock(spec=Context)
        main.body(ctx)
        mock_run_command.assert_called_once_with(ctx, "inv --list", "To select command type invoke again and cmd", "fail")
