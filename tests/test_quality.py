...
class TestQuality(unittest.TestCase):
    ...
    def test_run_tests(self, mock_run_command):
        ctx = MagicMock(spec=Context)
        run_tests.body(ctx)
        mock_run_command.assert_called_once_with(ctx, "pytest --cov=src --cov-report=term-missing", "Tests passed", "Tests failed")

    @patch('src.quality.run_command')
    def test_security(self, mock_run_command):
        ctx = MagicMock(spec=Context)
        security.body(ctx)
        mock_run_command.assert_called_once_with(ctx, "bandit -r .", "Security checks passed", "Security checks failed")
