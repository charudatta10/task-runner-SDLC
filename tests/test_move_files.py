...
class TestMoveFiles(unittest.TestCase):
    ...
    def test_create_patterns_sample(self, mock_json_dump, mock_file_open, mock_print):
        ...
        mock_file_open.assert_called_once_with(
            "test_patterns.json", "w", encoding="utf-8"
        )
        mock_json_dump.assert_called_once()
        mock_print.assert_called_once_with(
            "✅ Sample patterns file created at: test_patterns.json"
        )
    ...
    def test_clean_folder(self, mock_organize_files_body):
        ...
        mock_organize_files_body.assert_called_once_with(
            ctx, "./test_folder", "./dest_folder", "my_patterns.json"
        )
    ...
    def test_clean_folder_default_destination(self, mock_organize_files_body):
        ...
        mock_organize_files_body.assert_called_once_with(
            ctx, "./test_folder", "./test_folder", "my_patterns.json"
        )
    ...
    def test_organize_files(
        ...
        mock_validate_paths.assert_called_once_with(
            "/source", "/destination", "/patterns.json"
        )
        ...
        mock_move_files_to_directory.assert_has_calls(
            [
                call(
                    mock_source_path,
                    ["*.jpg", "*.png"],
                    mock_dest_path / "Images",
                    mock_logger,
                ),
                call(
                    mock_source_path,
                    ["*.pdf"],
                    mock_dest_path / "Documents",
                    mock_logger,
                ),
            ],
            any_order=True,
        ...
        mock_logger.info.assert_has_calls(
            [
                call(
                    f"Starting file organization from: {mock_source_path} to: {mock_dest_path}"
                ),
                ...
                call(
                    "File organization completed. Total: 0 moved, 0 failed"
                ),  # Default return from mock_move_files_to_directory
            ],
            any_order=True,
        )
    ...
    @patch(
        "src.tools.move_files.Path.resolve",
    ...
    def test_validate_paths_success(
        self, mock_mkdir, mock_resolve, mock_is_dir, mock_exists
    ):
    ...
    @patch("builtins.print")
    def test_validate_paths_source_not_directory(
        self, mock_print, mock_is_dir, mock_exists
    ):
    ...
    @patch(
        "src.tools.move_files.Path.exists", side_effect=[True, True, False]
    )  # source exists, dest exists, patterns not exists
    @patch("src.tools.move_files.Path.is_dir", return_value=True)
    @patch("builtins.print")
    def test_validate_paths_patterns_not_found(
        self, mock_print, mock_is_dir, mock_exists
    ):
        with self.assertRaises(FileNotFoundError):
    ...
    @patch(
        "src.tools.move_files.get_unique_filename",
        side_effect=["file1.txt", "file2_v1.txt"],
    )
    @patch("shutil.move")
    @patch("builtins.print")
    @patch("logging.info")
    @patch("logging.error")
    def test_move_files_to_directory(
        self,
        mock_error,
        mock_info,
        mock_print,
        ...
        moved, skipped = move_files_to_directory(
            source_path, ["*.txt"], destination_path, logger
        )
        ...
        mock_info.assert_has_calls(
            [
                ...
                call(
                    "MOVED (renamed): file2.txt -> {}{} (renamed due to duplicate)".format(
                        destination_path, "file2_v1.txt"
                    )
                ),
            ]
        )
