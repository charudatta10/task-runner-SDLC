...
def select_github_repo(ctx, base_path="C:/Users/korde/Home/Github"):
    ...
    """
    folders = [d for d in Path(base_path).iterdir() if d.is_dir()] # Get all folders in the directory
...
def move_files(ctx, directory=None):
    ...
    file_patterns = {
        "images": ["*.png", "*.jpg", "*.jpeg", "*.gif", "*.bmp", "*.tiff", "*.webp", "*.svg", "*.ico", "*.eps", "*.raw", "*.drawio", "*.avif"],
        "documents": ["*.pdf", "*.doc", "*.docx", "*.xls", "*.xlsx", "*.ppt", "*.pptx", "*.txt", "*.rtf", "*.csv", "*.xml", "*.json", "*.md", "*.tex", "*.odt", "*.ods", "*.odp", "*.epub", "*.mobi", "*.azw3", "*.fb2"],
        "archives": ["*.zip", "*.rar", "*.7z", "*.tar", "*.gz", "*.bz2", "*.xz"],
        "code": ["*.py", "*.c", "*.cpp", "*.java", "*.js", "*.ts", "*.html", "*.css", "*.scss", "*.sass", "*.less", "*.exe", "*.msi", "*.app", "*.iso", "*.img", "*.deb", "*.rpm", "*.jar", "*.war", "*.ear", "*.dll", "*.so", "*.pkg", "*.run", "*.sh", "*.bat", "*.cmd", "*.vbs", "*.ps1", "*.psm1", "*.psd1", "*.ps1xml", "*.reg"],
        "music": ["*.mp3", "*.wav"],
        "videos": ["*.mp4", "*.m4a", "*.mov"]
    }
...
@task
def generate_system_reports(ctx, destination_folder=Path("C:/Users/korde/Home/Github/backup-list/list")):
    """
...
ns = Collection(select_github_repo, clone_github_repos, move_files, generate_system_reports)