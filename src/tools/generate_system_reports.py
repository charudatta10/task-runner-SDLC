...
def generate_system_reports(ctx, destination_folder=None, include_optional=False):
    ...
    try:
            ...
            try:
                ...
                if result.ok:
                    ...
                    for env_path in envs:
                        ...
                        try:
                            env_result = ctx.run(f"conda env export --name {env_name} --json", 
                                               hide=True, warn=True)
                            if env_result.ok:
                                conda_exports[env_name] = json.loads(env_result.stdout)
                                logger.info(f"✅ Exported conda environment: {env_name}")
                                print(f"✅ Exported conda environment: {env_name}")
                        except Exception as e:
                            logger.warning(f"⚠️ Failed to export conda env {env_name}: {e}")
        ...
        for config in config_files:
            if config["source"]:
                if copy_file_safe(config["source"], config["destination"], config["description"]):
                    copied_files += 1
...
def list_system_tools(ctx):
    ...
    """
    tools_to_check = [
        "scoop", "winget", "pip", "pipx", "conda", "npm", 
        "choco", "brew", "cargo", "gem", "git", "node", "python"
    ]
